...
comparison_full <- comparison_full %>%
  mutate(
    sig_ISS = ifelse(padj_ISS < 0.00001, "Significant", "Not_Significant"),
    sig_WTD = ifelse(padj_WTD < 0.00001, "Significant", "Not_Significant"),
...
```{r}
EnhancedVolcano(res_ISS, lab = res_ISS$gene, x = "log2FoldChange_ISS", y = "padj_ISS", pCutoff = 0.00001)
```

```{r}
EnhancedVolcano(res_WTD, lab = res_ISS$gene, x = "log2FoldChange_WTD", y = "padj_WTD", pCutoff = 0.00001)
```
