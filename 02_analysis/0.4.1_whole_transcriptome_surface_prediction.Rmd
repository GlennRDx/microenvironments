...
# 3. Normalise
```{r}
# Manual CPM Normalisation + Log Transformation
...
# Transpose twice to divide columns (cells) by the library size vector efficiently
cpm_counts <- t(t(raw_counts) / lib_size) * 1e6

# 4. Log transform: log(CPM + 1)
# Using log1p (natural log) to match standard Seurat conventions
log_cpm_counts <- log1p(cpm_counts)

# 5. Inject back into the 'data' layer
...
ggsave(
  filename = file.path(fig_subdir, paste0("scRNA_LDA_projection_", classification_method, ".png")), plot = p_pred, dpi = 300)
```
