...
- Remove cells from cluster 8 (presumed dead or dying)
- 
```{r}
...
```{r}
# Proportions of each class in the train and test set
prop.table(table(inv_gene_exp_train$surface_class))

prop.table(table(inv_gene_exp_test$surface_class))
```
...
```{r}
# Separate training genes
inv_tumour_expr <- inv_tumour_expr %>%
  dplyr::select(surface_class, all_of(train_genes))

# Split into X (features) and Y (labels) for efficient processing
x_train <- inv_tumour_expr %>% dplyr::select(-surface_class)
y_train <- inv_tumour_expr$surface_class
