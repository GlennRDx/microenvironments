...
radii <- seq(10, 30, by = 2.5)      # Adjust range as needed
thresholds <- seq(0.7, 0.95, by = 0.05) # Adjust range as needed
...
results <- map_df(1:nrow(grid), function(i) {
  ...
  ggsave(file.path(spatial_plots_dir, paste0(test_id, "_spatial.png")), plot = p_spatial, width = 13.7, height = 11.3)
  ...
  # Return more metrics for the heatmaps
  return(data.frame(
    radius = curr_r, 
    threshold = curr_t, 
    balanced_accuracy = mean(cv_metrics$`Balanced Accuracy`, na.rm = TRUE),
    sensitivity = mean(cv_metrics$Sensitivity, na.rm = TRUE),
    specificity = mean(cv_metrics$Specificity, na.rm = TRUE)
  ))
})

# 4. FINAL GRID PLOTS
# Helper function to generate standardized heatmaps
make_heatmap <- function(data, fill_var, title, label) {
  ggplot(data, aes(x = factor(radius), y = factor(threshold), fill = .data[[fill_var]])) +
    geom_tile(color = "white") +
    scale_fill_viridis_c(option = "magma") +
    geom_text(aes(label = round(.data[[fill_var]], 3)), color = "white", size = 3) +
    labs(title = title, x = "Search Radius", y = "Tumor Fraction Threshold", fill = label) +
    theme_minimal()
}

# Requirement 2: Grid plot of (Sensitivity + Specificity) / 2
p_bal_acc <- make_heatmap(results, "balanced_accuracy", "Optimization: Balanced Accuracy", "Bal. Acc")
ggsave(file.path(opt_dir, "heatmap_balanced_accuracy.png"), p_bal_acc, width = 8, height = 6)

# Requirement 3: Grid plot of Sensitivity
p_sens <- make_heatmap(results, "sensitivity", "Optimization: Sensitivity (Core Prediction)", "Sens")
ggsave(file.path(opt_dir, "heatmap_sensitivity.png"), p_sens, width = 8, height = 6)

# Requirement 4: Grid plot of Specificity
p_spec <- make_heatmap(results, "specificity", "Optimization: Specificity (Surface Prediction)", "Spec")
ggsave(file.path(opt_dir, "heatmap_specificity.png"), p_spec, width = 8, height = 6)
```
