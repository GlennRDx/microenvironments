...
iss_obj_invasive <- iss_obj_invasive %>%
  ...
  FindClusters(resolution = 0.6, algorithm = 4, verbose = F) %>%
...
for (current_cluster in clusters) {
  ...
  # Define the specific color for this cluster
  # We wrap in as.character to ensure safe matching with named vector
  cluster_color <- turbo_cols[as.character(current_cluster)]
...
Idents(iss_obj_invasive) <- "surface_class_nn"
...
# GO Enrichment
```{r}
# 1. Select genes: Significant & Upregulated in Surface
genes_of_interest <- rownames(subset(sig_markers, avg_log2FC > 0.25))

# 2. Run GO Enrichment (Biological Process)
...
# 3. Visualise top 15 terms
dotplot(go_result, showCategory = 15) + 
  ggtitle("GO Enrichment: Upregulated in Surface") +
