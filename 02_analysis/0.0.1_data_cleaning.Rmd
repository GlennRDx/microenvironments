...
# 0.1.1 - scRNA-seq Data Cleaning
1. Import files:  
- Read in the expression data and meta data as dataframes

2. Expression Data Cleaning:  
- Restructure (transpose) expression data so rows are genes, columns are cells.
- Map cell barcodes from meta data to expression data (this is risky as we're not positive that they are in the correct order)

3. Meta Data Cleaning:
- Clean the dataframe by assigning the barcodes to the rownames instead of a "Barcode" column.
- Create "Cancerous" column by labelling the cancerous cell types as "Cancerous"

4. Use two files to create Seurat object and save the three files (expr, meta, and sobj) in RDS format.
...
# Assign the cell barcodes to the rownames instead of individual column "Barcode" column
...
# 0.2.2 - ISS data
1. Import files:  
- Read in the expression data and two metadata files (ISS_cell_types, and ISS_cells) as dataframes

2. Expression Data Cleaning:  
- Set rownames to gene names using the first column then delete the first column.

3. Meta Data Cleaning:  
- Meta data is stored in two separate files "ISS_cell_types.txt" and "ISS_cells.csv".  
- Clean first file so it is a single column of cell types.  
- Now add this to the other meta data file which contains cell coords (and other metadata).  
...
# 0.3.1
- Find the genes shared between the two datasets and save to an RDS file. 
```{r}
