...
ggsave(
  filename = file.path(fig_subdir, "p1_dist_total_counts.tiff"),
  plot = p1,
  width = 7.87, 
  height = 6,
  dpi = 300,
  compression = "lzw" # Good for TIFF size
)
```
...
ggsave(
  filename = file.path(fig_subdir, "p2_pct_MT_genes.tiff"),
  plot = p2,
  width = 7.87, 
  height = 6,
  dpi = 300,
...
ggsave(
  filename = file.path(fig_subdir, "p3_seq_depth_vs_num_detected_genes.tiff"),
  plot = p3,
  width = 7.87, 
  height = 6,
  dpi = 300,
...
# wts_obj <- SCTransform(wts_obj)
head(GetAssayData(wts_obj, layer = "data")) # confirm normalised
```
```{r}
# Feature Selection (Binomial Deviance)

# Convert Seurat object to single cell experiment (SCE) for compatibility with scry
sce <- as.SingleCellExperiment(wts_obj, assay = 'RNA')

# Calculate the binomial deviance (of each gene?) - use counts layer (raw counts) as the deviance is modeled on raw integer counts
sce <- devianceFeatureSelection(sce, assay = "counts")

# Extract deviance values and rank genes (Higher deviance = more deviation from the Null mode i.e. more biologically informative)
bin_dev <- rowData(sce)$binomial_deviance
names(bin_dev) <- rownames(sce)

# Add deviance values to the Seurat Assay metadata (using the [[]]) on the assay ensures compatibility with Seurat v4 and v5
wts_obj[["RNA"]][["binomial_deviance"]] <- bin_dev

# Rank and select the top 4000 deviant genes
top_4000 <- names(sort(binomial_deviance, decreasing = TRUE)[1:4000])

# Create the logical vector for the top 4000 genes
is_deviant <- rownames(wts_obj) %in% top_4000

# Name the logical vector so Seurat can map it to the features
names(is_deviant) <- rownames(wts_obj)

# Assign the logical vector to the Seurat object
wts_obj[["RNA"]][["highly_deviant"]] <- is_deviant

# Set Variable Features
VariableFeatures(wts_obj) <- top_4000

# Verify the metadata columns were created
head(wts_obj[["RNA"]][[]])
```
...
normalised_counts <- GetAssayData(wts_obj, layer = "data", assay = "RNA")
...
```{r, fig.width=7.87, dpi=300, dev='tiff', out.width='100%'}
# t-SNE
wts_obj <- RunTSNE(wts_obj, dims = wts_params$n_dims, reduction = "pca", verbose = F) 
...
# UMAP
#calc neighbours
wts_obj <- FindNeighbors(wts_obj, dims = wts_params$n_dims, reduction = "pca", verbose = F)

# Run UMAP
wts_obj <- RunUMAP(wts_obj, dims = wts_params$n_dims, reduction = "pca", verbose = F)
