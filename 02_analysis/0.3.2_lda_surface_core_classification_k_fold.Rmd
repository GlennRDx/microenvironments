...
for(i in 1:5) {
  ...
  train_data_filt <- train_data %>% dplyr::select(surface_class, all_of(train_genes))
  test_data_filt  <- test_data  %>% dplyr::select(all_of(train_genes))
...
# 9. Train final LDA model using all cells
```{r}
# Filter for training genes
final_train_data <- inv_tumour_expr %>%
  dplyr::select(surface_class, all_of(train_genes))

# Fit Final LDA
fit_lda_final <- lda(
  x = final_train_data %>% dplyr::select(-surface_class), 
  grouping = final_train_data$surface_class
)
