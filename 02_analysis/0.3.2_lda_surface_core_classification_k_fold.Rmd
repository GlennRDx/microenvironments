...
# CRITICAL: Drop unused factor levels
# If we don't do this, R will still think "Other" is a possible category with 0 cells
...
# Dynamically assign the class column based on config
...
# Ensure surface_class is a factor for caret/LDA
inv_tumour_expr$surface_class <- as.factor(inv_tumour_expr$surface_class)
```
...
saveRDS(fit_lda_final, file = "janesick_2023/processed/fit_lda_final.RDS")
saveRDS(final_genes, file = "janesick_2023/processed/genes_to_keep_final.RDS")
```
...
ggplot(plot_data, aes(x = y_centroid, y = x_centroid, color = prob_surface)) +
  ...
  coord_fixed(ylim = c(3850, 0)) +
  theme_minimal() +
  labs(title = "LDA Surface Probability Map", subtitle = "White = High Uncertainty (Transition Zone)")
```
