...
p_bias_check <- plot_data %>%
  filter(!is.na(predicted_surface_class)) %>%
  ggplot(aes(x = predicted_surface_class, y = nCount_RNA, fill = predicted_surface_class)) +
...
barplot(table(wts_obj_invasive$predicted_surface_class, cut(wts_obj_invasive$nCount_RNA, 200)),
        legend = TRUE, 
        las = 2, 
...
```{r}
barplot(prop.table(table(wts_obj_invasive$predicted_surface_class, cut(wts_obj_invasive$nCount_RNA, 200)), 2),
        legend = TRUE, 
        las = 2, 
