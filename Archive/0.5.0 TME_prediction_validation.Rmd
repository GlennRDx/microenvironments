...
# Load the exact genes used in LDA training
genes_to_keep <- readRDS("/home/glennrdx/Documents/lab_projects/microenvironments/01_data/janesick_2023/processed/genes_to_keep.RDS")
...
cell_assignments <- sample(rep(1:n, length.out = n_cells))
...
# Filter to only genes used in LDA training (genes_to_keep)
counts_WTD <- counts_WTD[rownames(counts_WTD) %in% genes_to_keep, ]
...
# Filter to only genes used in LDA training (genes_to_keep)
counts_ISS <- counts_ISS[rownames(counts_ISS) %in% genes_to_keep, ]
...
# CREATE comparison variable for TRAINING genes
...
```{r}
# Redefine comparison for VALIDATION genes
