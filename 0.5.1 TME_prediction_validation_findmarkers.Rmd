...
# Load ISS / scRNA-seq data
iss_obj_invasive <- readRDS("/home/glennrdx/Documents/lab_projects/microenvironments/01_data/janesick_2023/processed/iss_obj_invasive.RDS")
scRNA_invasive <- readRDS('/home/glennrdx/Documents/lab_projects/microenvironments/01_data/janesick_2023/processed/wts_obj_invasive.RDS')


# Load shared genes and training genes
shared_genes <- readRDS("/home/glennrdx/Documents/lab_projects/microenvironments/01_data/janesick_2023/processed/shared_genes.RDS")
train_genes <- readRDS("/home/glennrdx/Documents/lab_projects/microenvironments/01_data/janesick_2023/processed/train_genes.RDS")

# Select classification method
# classification_method <- "surface_class_clusters"
classification_method <- "surface_class_radius"

# findmarkers_method <- "wilcox"
findmarkers_method <- "MAST"
# findmarkers_method <- "DESeq2"
```
...
cor_pearson_training <- cor(
  comparisons_training_sig$log2FoldChange_ISS,
  comparisons_training_sig$log2FoldChange_WTD,
  use = "complete.obs"
)
cat("Training genes - Pearson r (sig genes):", round(cor_pearson_training, 3), "\n")

# Validation genes correlation
cor_pearson_validation <- cor(
  comparisons_validation_sig$log2FoldChange_ISS,
  comparisons_validation_sig$log2FoldChange_WTD,
  use = "complete.obs"
)
cat("Validation genes - Pearson r (sig genes):", round(cor_pearson_validation, 3), "\n")
...
p1 <- ggplot(comparison_training, aes(x = log2FoldChange_ISS, y = log2FoldChange_WTD)) +
  ...
  labs(
    title = "FindMarkers Results: ISS vs WTD TRAINING GENES",
    subtitle = paste0("Pearson r = ", round(cor_pearson_training, 3),
                     " (n=", nrow(comparisons_training_sig), " mutually sig)"),
...
p2 <- ggplot(comparison_validation, aes(x = log2FoldChange_ISS, y = log2FoldChange_WTD)) +
  ...
  labs(
    title = "FindMarkers Results: ISS vs WTD VALIDATION GENES",
    subtitle = paste0("Pearson r = ", round(cor_pearson_validation, 3),
                     " (n=", nrow(comparisons_validation_sig), " mutually sig)"),
    x = "log2 Fold Change (ISS)",
    y = "log2 Fold Change (WTD)",
    color = "Significance Status"
  ) +
