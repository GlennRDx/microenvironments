...
```{r}
# Parameters
R <- 35  # Radius for neighbourhood definition
K <- 25 # Number of neighbours for KNN method
center_x <- 3249.27 # random cell x coord
center_y <- 2027.26 # random cell y coord
...
nn_result <- nn2(data = coords, query = coords, searchtype = "radius", radius = R, k = 200)
...
classification_vec <- vapply(invasive_idx, function(i) {
  ...
  if (frac > THRESHOLD) "Core" else "Surface"
...
# 2. KNN Search
nn_knn <- nn2(data = coords_all, query = coords_all[invasive_idx,], k = K)
